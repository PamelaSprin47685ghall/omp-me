...
- [x] `squad-tau/` 完整源码（server + client + shared + test）
  ...
  - [x] 前端：React SPA（绝对贫血，无 useState 业务状态）、Chakra UI 组件、event-store（`useSyncExternalStore` 原子订阅）
...
- [x] 幽灵 PTY 物理仿真（`simulation.js` + `test/e2e/`）
  - [x] `simulation.js` — 无磁盘、无轮询的 PTY 直连全链路测试
  - [x] `chaos-ui-e2e.test.js` — 浏览器端混沌测试
...
- 流式渲染：RAF 双缓冲 + `joinedText`/`joinedThinking` 预缓存，零数组遍历
...
| 浏览器性能瓶颈（大量消息） | 中 | delta 渲染 + RAF 双缓冲 + `joinedText`/`joinedThinking` 缓存 |
...
| 14 | UI: 流式渲染 | RAF 双缓冲 + `joinedText`/`joinedThinking` 预缓存 |
...
| 10 | 流式渲染 | React State 频繁 setState | RAF 双缓冲直接写入 DOM + `joinedText`/`joinedThinking` 预缓存 |
| 11 | 外层 review 重置 | 手动节点重置逻辑 | Reactor 自动推导 |
