 ...
 *
 * Run explicitly:
 *   SQUAD_MODEL=p-openai/gpt-5.2 bun test ./simulation/simulation.js
 */
import { describe, test, expect, beforeAll, afterAll } from 'bun:test';
import path from 'path';
import { readdirSync, readFileSync, existsSync, statSync } from 'fs';
import { setupBrowser, teardownBrowser } from '../test-ported/helpers/puppeteer-setup.js';
...
describe('Squad Physical Simulation', () => {
    ...
    test('squad completes via WebSocket event and writes artifact', async () => {
        ...
        // Zero-disk-IO assertion: no unexpected temp files
        // Expected: the squad-events.ndjson, any .js artifacts (calculator program),
        // and the .omp/ directory for plan files.
        // Forbidden: .tmp, .lock, .db, .swp, random temporary directories.
        const allFiles = readdirSync(testDir);
        const forbidden = allFiles.filter(
            (f) =>
                /\.(tmp|lock|db|swp|bak|log|pid)$/.test(f) ||
                /^\.omp\/.*\.(tmp|lock|db|swp|bak|log|pid)$/.test(f) ||
                f.startsWith('.nfs') ||
                /squad-physical-.*-\d+\.(tmp)$/.test(f),
        );
        expect(forbidden.length).toBe(0, `Zero unexpected temp files: found ${JSON.stringify(forbidden)}`);

        // Verify omp directory only contains expected files
        const ompDir = path.join(testDir, '.omp');
        if (existsSync(ompDir)) {
            const ompEntries = readdirSync(ompDir, { recursive: true });
            const forbiddenOmp = ompEntries.filter((f) => /\.(tmp|lock|db|swp|bak|log|pid)$/.test(f));
            expect(forbiddenOmp.length).toBe(
                0,
                `.omp/ has zero unexpected temp files: found ${JSON.stringify(forbiddenOmp)}`,
            );
        }
    }, 620000);
});
