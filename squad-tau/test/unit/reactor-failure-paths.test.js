...
describe('reviewer rejection', () => {
    ...
    test('retryCount increments on repeat rejections', () => {
        ...
        let a = reactState(st).find((e) => e.type === Events.SQUAD_NODE_STATE && e.payload.status === STATUS.AUTHORING);
        expect(a.payload.retryCount).toBe(1);
        ...
        a = reactState(st).find((e) => e.type === Events.SQUAD_NODE_STATE && e.payload.status === STATUS.AUTHORING);
