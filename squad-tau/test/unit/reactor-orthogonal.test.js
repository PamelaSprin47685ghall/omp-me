...
describe('max retries boundary', () => {
    test('after MAX_RETRIES consecutive reviewer rejections, node goes to FAILED', () => {
        ...
        const fail = actions.filter((a) => a.type === Events.SQUAD_NODE_STATE && a.payload.status === 'failed');
        expect(fail.length).toBe(1);
        expect(fail[0].payload.nodeId).toBe('n1');
        const auth = actions.filter((a) => a.type === Events.SQUAD_NODE_STATE && a.payload.status === 'authoring');
    ...
    test('at MAX_RETRIES-1 retries, rejection sends back to AUTHORING', () => {
        ...
        const actions = reactState(st);
        const auth = actions.find((a) => a.type === Events.SQUAD_NODE_STATE && a.payload.status === 'authoring');
