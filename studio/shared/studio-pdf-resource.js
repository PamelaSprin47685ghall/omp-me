...
export function resolveStudioPdfResourceFile(pdfPath, baseDir) {
  ...
  if (/^[a-z][a-z0-9+.-]*:/i.test(rawPath) && !/^[a-z]:[\\/]/i.test(rawPath)) throw new Error("Only local PDF paths are supported.");
  ...
  if (rel.startsWith("..") || isAbsolute(rel)) throw new Error("PDF path must stay within the current Studio resource directory.");
  const st = statSync(candidateReal);
  if (!st.isFile()) throw new Error("PDF path does not refer to a file.");
