...
export function handleLoopControlTool(params, state) {
	...
	if (params.status === "done") {
		return {
			content: [{ type: "text", text: state.status === "running" ? buildConfirmMessage(state) : buildDoneMessage(state, params.summary) }],
		};
	}
...
export function createRenderCall(Text) {
	return (args, theme) =>
		new Text(theme.fg("toolTitle", theme.bold("loop_control ")) + theme.fg(args.status === "done" ? "success" : "accent", args.status), 0, 0);
}
