...
export default async function reviewLoopExtension(pi) {
	...
	try {
		...
		pi.registerTool({
			...
			async execute(_id, params, _signal, _onUpdate, ctx) { return fsm.executeTool(_id, params, _signal, _onUpdate, ctx); },
		...
		pi.registerShortcut("ctrl+shift+s", {
			description: "Stop the active loop",
			handler: async (ctx) => { fsm.stop(ctx, "Stopped by shortcut"); ctx.abort(); },
		});
