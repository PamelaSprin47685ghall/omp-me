...
export function transition(state, event) {
	switch (event.type) {
		...
		case "stop": {
			...
			return {
				state: { status: "done", step: state.step, goal: state.goal, reasonDone: event.reason, lastSummary: state.lastSummary },
				effects: { status: true },
			};
		}
		...
		case "done_request": {
			...
			return { state: { status: "confirming", step: state.step, goal: state.goal, reasonDone: s, lastSummary: s }, effects: {} };
		}

		case "continue": {
			if (state.status !== "confirming") return { state, effects: {} };
			return { state: { status: "running", step: state.step, goal: state.goal, lastSummary: event.summary.trim() }, effects: {} };
		}
		...
		case "done_confirm": {
			...
			return { state: { status: "done", step: state.step, goal: state.goal, reasonDone, lastSummary: summary || state.lastSummary }, effects: { status: true } };
		}
