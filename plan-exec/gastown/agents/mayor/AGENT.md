...
3. Assign agents based on role:
   - crew-lead: persistent collaborator for complex, multi-bead work requiring continuity
   - polecat: transient task worker for isolated, single-bead tasks with clean teardown
   - witness: per-rig lifecycle manager for overseeing sessions and local state
...
7. Coordinate integration review via Refinery
```
