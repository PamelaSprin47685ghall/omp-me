...
async function main(task) {
    ...
    for (let cycle = 1; cycle <= maxCycles; cycle++) {
        ...
        // 预测性告警
        if (cycle > 1) {
            const trend = await task(`Patrol Cycle ${cycle} 趋势分析：对比上一轮`, {
                type: 'object',
                properties: { unhealthyGrowth: { type: 'number' } },
            });
            if (trend.unhealthyGrowth > 0)
                alerts.push({ cycle, severity: 'predictive', msg: 'unhealthy trend rising' });
        }
