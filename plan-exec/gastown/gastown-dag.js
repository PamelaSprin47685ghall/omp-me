...
async function main(task) {
  ...
  const graph = await task('返回要执行的 DAG：nodes 和 edges', {
    type: 'object',
    properties: {
      nodes: { type: 'array', items: { type: 'object', properties: { id: {type: 'string'}, run: {type: 'string'} } } },
      edges: { type: 'array', items: { type: 'object', properties: { from: {type: 'string'}, to: {type: 'string'}, predicate: {} } } },
    }
  })
  ...
  // Kahn + 并行调度
  const done = new Set(), fail = new Set(), skip = new Set(), out = {}
  ...
  while (ready.length) {
    ...
    await Promise.all(next.map(async id => {
      ...
      if (!passes) { skip.add(id); out[id] = { skipped: true }; return }
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          out[id] = node.run
            ? await task(`执行 DAG 节点 ${id}，输入：${JSON.stringify(Object.fromEntries(upstream.map(e => [e.from, out[e.from]])))}`, { type: 'object' })
            : {}
          done.add(id); return
        } catch (err) {
          lastErr = err
      ...
      for (const e of edges.filter(e => e.from === id)) {
        if (!fail.has(e.to)) { fail.add(e.to); out[e.to] = { failed: true, error: 'upstream failed' } }
      }
    }))
  ...
  return await task(`DAG 执行结果：order=${JSON.stringify([...done, ...skip])}, failed=${JSON.stringify([...fail])}`, {
    type: 'object',
    properties: { ok: {type: 'boolean'}, order: {type: 'array'}, failed: {type: 'array'}, out: {type: 'object'} }
  })
}
