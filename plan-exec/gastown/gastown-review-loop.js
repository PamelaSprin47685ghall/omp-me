...
async function main(task) {
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    // 审查者并行审查
    const reviews = await Promise.all(reviewers.map((r, i) =

      task(`Reviewer ${i} 审查 Round ${round} 产出：${JSON.stringify(deliverable)}`, {
        type: 'object',
        properties: { verdict: {type: 'string', enum:['accept','reject']}, comments: {type:'array'}, confidence: {type:'number'} }
      })
    ))
    ...
    switch (config.quorumMode) {
      case 'unanimity': ok = accepts === total; break
      case 'majority':  ok = accepts > total / 2; break
      default:          ok = accepts === total
    }
  ...
  return await task(`Review Loop 完成：accepted=${accepted}, rounds=${history.length}`, {
    type: 'object',
    properties: { accepted: {type: 'boolean'}, rounds: {type: 'number'}, deliverable: {}, history: {type: 'array'} }
  })
}
