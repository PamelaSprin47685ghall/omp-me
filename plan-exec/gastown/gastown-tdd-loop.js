...
async function main(task) {
    ...
    for (let c = 1; c <= maxCycles; c++) {
        ...
        // RED
        cycle.red = red ? await task(`TDD Cycle ${c} RED：编写未通过的测试`, { type: 'object' }) : {};
        const pre = testRunner
            ? await task(`TDD Cycle ${c} RED-Check：验证测试确实失败`, {
                  type: 'object',
        ...
        // GREEN
        cycle.green = green
            ? await task(`TDD Cycle ${c} GREEN：最小实现使测试通过。测试：${JSON.stringify(cycle.red)}`, {
                  type: 'object',
              })
            : {};
        const greenCheck = testRunner
            ? await task(`TDD Cycle ${c} GREEN-Check：验证实现通过测试`, {
                  type: 'object',
                  properties: { passed: { type: 'boolean' } },
              })
            : { passed: true };
        ...
        // REFACTOR
        if (refactor) {
            cycle.refactor = await task(`TDD Cycle ${c} REFACTOR：重构代码。实现：${JSON.stringify(cycle.green)}`, {
                type: 'object',
            });
            const refactorCheck = testRunner
                ? await task(`TDD Cycle ${c} REFACTOR-Check：重构后测试仍通过`, {
                      type: 'object',
                      properties: { passed: { type: 'boolean' } },
                  })
                : { passed: true };
        ...
        if (testRunner && config.coverageThreshold) {
            const cov = await task(`TDD Cycle ${c} 覆盖率检查：threshold=${config.coverageThreshold}`, {
                type: 'object',
