...
async function main(task) {
    ...
    switch (config.mode) {
        case 'unanimity':
            passed = tally.yes === tally.total;
            break;
        case 'majority':
            passed = tally.yes > effective / 2;
            tie = tally.yes * 2 === effective;
            break;
        case 'threshold':
            passed = tally.yes / Math.max(effective, 1) >= (config.threshold ?? 0.5);
            break;
        case 'any-of':
            passed = tally.yes >= 1;
            break;
        case 'custom':
            passed = await task(`自定义门控判断：${JSON.stringify(tally)}`, { type: 'boolean' });
            break;
        default:
            passed = tally.yes > effective / 2;
    }
    ...
    if (tie) {
        ...
        passed =
            resolver !== undefined ? resolver : await task(`平局裁决：${JSON.stringify(tally)}`, { type: 'boolean' });
    }
