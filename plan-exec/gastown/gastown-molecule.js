...
async function main(task) {
    ...
    while (i < steps.length) {
        ...
        switch (step.type) {
            case 'task':
                result = await task(`执行分子步骤 ${i}（task）`, { type: 'object' });
                break;
            case 'dag':
                result = await task(`执行分子步骤 ${i}（dag）`, { type: 'object' });
                break;
            case 'gatekeeper':
                result = await task(`执行分子步骤 ${i}（gatekeeper）`, { type: 'object' });
                break;
            case 'review':
                result = await task(`执行分子步骤 ${i}（review）`, { type: 'object' });
                break;
            case 'tdd':
                result = await task(`执行分子步骤 ${i}（tdd）`, { type: 'object' });
                break;
            case 'loop':
                result = await task(`执行分子步骤 ${i}（loop）`, { type: 'object' });
                break;
            default:
