...
function buildProgressPayload(sessionId, codeLines, lineIdx, modelPool) {
    ...
    return {
        forks: forkInfos,
        codeLine,
        slotUsage: modelPool ? { used: modelPool.busyCount, total: modelPool.totalSlots } : undefined,
    };
}
...
function emitProgress(ctx, sessionId, onUpdate, codeLines, lineIdx, modelPool) {
    ...
    try {
        ctx?.ui?.notify?.(formatNotifyMessage(progress), 'info');
    } catch {}
}
...
function buildSessionOptions(ctx, pi, returnTool, parentDepth, modelOverride) {
    ...
    if (modelOverride) {
        ...
        const matched = available.find((m) => m.provider === modelOverride.provider && m.id === modelOverride.id);
        if (matched) {
    ...
    const activeTools = (ctx?.session?.getActiveToolNames?.() ?? pi?.getActiveTools?.())?.filter((t) => t !== 'task');
...
async function spawnTaskFork(
    ...
    if (!createAgentSession) {
        throw new Error('plan_exec engine not initialized: createAgentSession unavailable');
    }
    ...
    try {
        ...
        if (!resolved && emptyTurnCount >= MAX_EMPTY_TURNS) {
            rejecter(new Error(`Task fork exited ${MAX_EMPTY_TURNS} times without calling return`));
        }
...
async function _executeUserCode(code, ctx, pi, signal, onUpdate, parentDepth, codeLines, modelPool, options = {}) {
...
export async function executePlan(code, ctx, pi, signal, onUpdate) {
    if (!pi?.pi?.createAgentSession) {
        throw new Error('plan_exec requires @oh-my-pi/pi-coding-agent but it is not available');
    }
