...
describe("createBridge", () => {
	...
	it("forwards sendUserMessage with image content array", async () => {
		...
		const bridge = createBridge(pi);
		bridge.sendUserMessage([{ type: "text", text: "hi" }, { type: "image", data: "abc", mimeType: "image/png" }]);
	...
	it("forwards setModel", async () => {
		...
		const pi = {
			on: () => {},
			setModel: () => { called = true; return Promise.resolve(true); },
		};
	...
	it("forwards setSessionName", async () => {
		...
		const pi = {
			on: () => {},
			setSessionName: (n) => { name = n; return Promise.resolve(); },
		};
